...
Effortlessly log daily calories, macros (Protein, Carbs, Fat, Fiber), and an intelligent **Nutrition & Fat Loss Score** (0–100) with **multi-photo album uploads (send multiple photos at once)**, complete data privacy, visual analytics, and smart editing.

---

## 🌟 Key Capabilities

### 1. Multi-Photo Album Logging (Send Multiple Photos as One Meal)
- 📸 **Upload Multiple Photos at Once**: Select and send 2–5 photos in a single Telegram message (e.g., multi-dish restaurant spreads, banchan, sides, drinks).
- 🥣 **Consolidated Single-Meal Evaluation**: The bot's `MediaGroupBuffer` coordinator debounces the album and passes all photos together to Gemini in **one unified vision session**. It logs the entire feast as **one consolidated meal** with zero duplicate entries and accurate calorie totals.
- 📝 **Album-Wide Caption Ground Truth**: Add a single caption to your album (e.g., *"5 items from Ajumma, half of each and one scoop peach gelato"*), and the AI applies the portion context across all photos.

### 2. Multi-Tenant Cloud Architecture & Data Privacy
